...
from .ctidh import hkdf, ctidh_parameters, ctidh
...
class Organize(attrdict):
    ...
    def ctidh_dh(self, pk):
        if self._ctidh_dh is None:
            ...
            ctidh = ctidh(ctidh_parameters)
            sk = self._keys.pq_ctidhP512_sec_key

            @memoize
            def _ctidh_dh(pk):
                self.log.debug("Generating CTIDH PSK for pk {}".format(pk))
                return ctidh.dh(sk, pk)

            self._ctidh_dh = _ctidh_dh
        raw_key = self._ctidh_dh(pk)
        # XXX To ensure that even if CTIDH is broken, we should integrate a DH from a
        # Curve-448 using a hybrid construction that is as secure as the most
        # secure of either (ctidh, curve448)
        psk = hkdf(raw_key)
        return psk
        ...
        return Descriptor(
            {
                ...
                "c": self._keys.pq_ctidhP512_pub_key, # XXX add Curve448 pk for hybrid DH
                "addrs": ip_addrs,
