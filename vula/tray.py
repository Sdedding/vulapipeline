...
def main() -> None:  # noqa: 901
    ...
    class Tray(object):
        ...
        def __init__(self):
            ...
            def _systemd_unit_path(name: str) -> str:
                """Return the DBus object path for a systemd unit."""
                return "/org/freedesktop/systemd1/unit/" + name.replace(
                    "_", "_5f"
                ).replace("-", "_2d").replace(".", "_2e")

            for service in ("publish", "discover", "organize"):
                unit = _systemd_unit_path(f"vula-{service}.service")
                self.system_bus.subscribe(
