...
class OrganizeBackend:
    ...
    def get_prefs(self) -> dict[str, Any]:
        if not self.connect():
            return {}
        try:
            return cast(dict[str, Any], json.loads(self.organize.get_prefs()))
        except Exception as exc:
    ...
    def our_latest_descriptors(self) -> str:
        if not self.connect():
            return "{}"
        try:
            return cast(str, self.organize.our_latest_descriptors())
        except Exception as exc:
    ...
    def verify_and_pin_peer(self, peer_vk: str, peer_name: str) -> None:
        if self.connect():
            try:
                ...
                self.log.error(
                    "DBus error verifying peer %s: %s", peer_vk, exc
                )
    ...
    def set_pref(self, pref: str, value: Any) -> Optional[str]:
        if not self.connect():
            return None
        try:
            return cast(str, self.organize.set_pref(pref, value))
        except Exception as exc:
            self.log.error("DBus error setting pref %s: %s", pref, exc)
            return None

    def add_pref(self, pref: str, value: Any) -> Optional[str]:
        if not self.connect():
            return None
        try:
            return cast(str, self.organize.add_pref(pref, value))
        except Exception as exc:
            self.log.error("DBus error adding pref %s: %s", pref, exc)
            return None

    def remove_pref(self, pref: str, value: Any) -> None:
        if self.connect():
            try:
