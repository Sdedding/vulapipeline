...
class PeerDetailsOverlay(tk.Toplevel):
    def __init__(self, parent: tk.Frame, peer: Peer, data: Controller) -> None:
    ...
    def edit_peer(self) -> None:
        try:
            assert self.peer.id is not None
    ...
    def add_additional_ip(self) -> None:
        ...
        try:
            assert self.peer.id is not None
            self.data.add_peer_ip(self.peer.id, self.entry_additional_ip.get())
